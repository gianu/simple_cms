...
var TinyMCE_AdvancedMediaPlugin = {
	getInfo : function() {
		return {
			longname : 'Advanced Media',
			author : 'Sean Naegle',
			authorurl : 'http://pullmonkey.com',
	...
	initInstance : function(inst) {
		...
		if (!tinyMCE.settings['media_skip_plugin_css'])
			tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/plugins/advmedia/css/content.css");
	},

	getControlHTML : function(cn) {
		switch (cn) {
			case "advmedia":
				return tinyMCE.getButtonHTML(cn, 'lang_advmedia_desc', '{$pluginurl}/images/media.gif', 'mceAdvMedia');
		}
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		...
		switch (command) {
			case "mceAdvMedia":
				tinyMCE.openWindow({
						file : '../../plugins/advmedia/media.htm',
						width : 430 + tinyMCE.getLang('lang_advmedia_delta_width', 0),
						height : 750 + tinyMCE.getLang('lang_advmedia_delta_height', 0)
					}, {
						editor_id : editor_id,
						inline : "yes"
				});
		...
		switch (type) {
				...
				for (i=0; i<nl.length; i++) {
					...
					switch (ci) {
						case 'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000':
							nl[i].parentNode.replaceChild(TinyMCE_AdvancedMediaPlugin._createImg('mceItemFlash', d, nl[i]), nl[i]);
							break;

						case 'clsid:166b1bca-3f9c-11cf-8075-444553540000':
							nl[i].parentNode.replaceChild(TinyMCE_AdvancedMediaPlugin._createImg('mceItemShockWave', d, nl[i]), nl[i]);
							break;

						case 'clsid:6bf52a52-394a-11d3-b153-00c04f79faa6':
						case 'clsid:22d6f312-b0f6-11d0-94ab-0080c74c7e95':
						case 'clsid:05589fa1-c356-11ce-bf01-00aa0055595a':
							nl[i].parentNode.replaceChild(TinyMCE_AdvancedMediaPlugin._createImg('mceItemWindowsMedia', d, nl[i]), nl[i]);
							break;

						case 'clsid:02bf25d5-8c17-4b23-bc80-d3488abddc6b':
							nl[i].parentNode.replaceChild(TinyMCE_AdvancedMediaPlugin._createImg('mceItemQuickTime', d, nl[i]), nl[i]);
							break;

						case 'clsid:cfcdaa03-8be4-11cf-b84b-0020afbbccfa':
							nl[i].parentNode.replaceChild(TinyMCE_AdvancedMediaPlugin._createImg('mceItemRealMedia', d, nl[i]), nl[i]);
							break;
					}
				}
				...
				for (i=0; i<nl.length; i++) {
					switch (tinyMCE.getAttrib(nl[i], 'type')) {
						case 'application/x-shockwave-flash':
							TinyMCE_AdvancedMediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemFlash');
							break;

						case 'application/x-director':
							TinyMCE_AdvancedMediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemShockWave');
							break;

						case 'application/x-mplayer2':
							TinyMCE_AdvancedMediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemWindowsMedia');
							break;

						case 'video/quicktime':
							TinyMCE_AdvancedMediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemQuickTime');
							break;

						case 'audio/x-pn-realaudio-plugin':
							TinyMCE_AdvancedMediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemRealMedia');
							break;
					}
				}
				break;
			...
			case "get_from_editor":
				...
				while ((startPos = content.indexOf('<img', startPos+1)) != -1) {
					...
					attribs = TinyMCE_AdvancedMediaPlugin._parseAttributes(content.substring(startPos + 4, endPos));
					...
					if (!tinyMCE.getParam('media_use_script', false)) {
						...
						embedHTML = TinyMCE_AdvancedMediaPlugin._getEmbed(ci, cb, mt, pl, attribs);
					} else {
						// Use script version
						switch (attribs['class']) {
							case 'mceItemFlash':
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		do {
			if (node.nodeName == "IMG" && /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(tinyMCE.getAttrib(node, 'class'))) {
				tinyMCE.switchClass(editor_id + '_advmedia', 'mceButtonSelected');
				return true;
			}
		} while ((node = node.parentNode));

		tinyMCE.switchClass(editor_id + '_advmedia', 'mceButtonNormal');
...
tinyMCE.addPlugin("advmedia", TinyMCE_AdvancedMediaPlugin);
